...
Display-only: shows what Stash is currently streaming. Stash exposes no
per-session metadata, so we cannot reliably attribute a stream to a specific
monitor or browser tab. Instead we surface the most-recently-active stream
as the primary media card and list every concurrent stream in attributes.
Playback controls are deliberately not exposed because Home Assistant cannot
actually drive the Stash web player.
"""
...
async def async_setup_entry(
    ...
    async_add_entities([StashMediaPlayer(entry, coordinator, client)])


class StashMediaPlayer(CoordinatorEntity, MediaPlayerEntity):
    """A single display-only player showing the most-recent active stream."""
    ...
    def __init__(self, entry: ConfigEntry, coordinator, client) -> None:
        ...
        self._default_name = player_name
    ...
    def _resolve_cover_entity(self) -> str | None:
        ...
        if self.hass is None:
            return None
        unique_id = f"{self._entry.entry_id}_cover"
